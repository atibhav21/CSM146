...
class PolynomialRegression() :
    ...
    def fit_GD(self, X, y, eta=None,
        ...
        for t in xrange(tmax) :
            ...
            error_array = y_pred - y
            update_array = np.dot(np.transpose(X), np.transpose(error_array))

            self.coef_ = self.coef_ - (2 * eta) * update_array
    ...
    def fit(self, X, y, l2regularize = None ) :
        ...
        inter_1 = np.linalg.pinv(np.dot(np.transpose(X), X))
        inter_2 = np.dot(inter_1, np.transpose(X))
        self.coef_ = np.dot(inter_2, y)
    ...
    def cost(self, X, y) :
        ...
        #X = self.generate_polynomial_features(X)
        error_array = np.square((np.dot(X, self.coef_) - y))
        cost = np.sum(error_array)
...
def main() :
    ...
    '''for eta in (0.01, 0.001, 0.0001, 0.0407):
        print(eta)
    ...
    start = time.time()
    model_2.fit(train_data.X, train_data.y)
    end = time.time()
    print "%.2gs" % (end - start)
    print 'Done with Closed form'

    print 'Trying out with Momentum'
    ...
    for m in range(0, 11):
        ...
        y_values_train.append(model.rms_error(train_data.X, train_data.y))
        y_values_test.append(model.rms_error(test_data.X, test_data.y))

    plt.plot(x_values, y_values_train, label="Training Error")
    plt.plot(x_values, y_values_test, label="Test Error")
